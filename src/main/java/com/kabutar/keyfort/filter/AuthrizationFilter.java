...
public class AuthrizationFilter implements WebFilter {
    ...
    public AuthrizationFilter(AuthConfig authConfig, AuthService authService) {
        ...
        this.patterns = this.authConfig.getPreAuthnUrls().stream()
                .map((AuthConfig.PreAuthnUrl url) -> parser.parse(url.getPath()))
                .toList();
    ...
    public Mono<Void> filter(ServerWebExchange exchange, WebFilterChain chain) {
        ...
        if (accessTokenCookie != null) {
            return authService
                    .validateAccessToken(accessTokenCookie.getValue())
                    .flatMap(map -> {
                        if ((boolean) map.get("isValid")) {
                            return chain.filter(this.getExchangeWithUser(exchange, (User) map.get("user")));
                        }
    ...
    private Mono<Void> sendErrorResponse(ServerHttpResponse response) {
        try {
            String responseBody =
                    this.objectMapper.writeValueAsString(Map.of("error", List.of("Unauthorized")));
