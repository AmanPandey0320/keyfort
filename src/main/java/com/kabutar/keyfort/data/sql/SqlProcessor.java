...
public class SqlProcessor {
    private static final Logger logger = LogManager.getLogger(SqlProcessor.class);
    /**
     * Generates an SQL UPDATE statement with named parameters based on entity fields. Updates all
     * fields annotated with @Column, excludes @Id fields from update set. Uses the @Id field as the
     * WHERE condition for the update.
     *
     * <p>Example output for Dimension entity: UPDATE dimensions SET name = :name, display_name =
     * :displayName, is_active = :isActive WHERE id = :id;
     *
     * @param entity The entity instance to analyze via reflection.
     * @return SqlQueryWithFields containing ordered fields (excluding @Id first, then @Id for
     *     where) and the update SQL string with named parameters.
     ...
     *
     * <p>This method inspects the provided {@code Class<?>} for the presence of the {@link Entity}
     * annotation and iterates through its fields (including inherited fields) to find those
     * annotated with {@link Column}. It constructs a `CREATE TABLE IF NOT EXISTS` statement based
     * on the table name specified in the {@link Entity} annotation and the column definitions from
     * the {@link Column} annotations.
     *
     * <p>The method handles fields from superclasses by traversing the class hierarchy until `null`
     * is reached. It ensures proper comma separation between column definitions.
     *
     * @param clazz The {@code Class<?>} object representing the entity for which the SQL CREATE
     *     statement is to be generated. This class must be annotated with {@link Entity}.
     ...
     *
     * <p>The statement excludes the {@code @Id} field (assuming it is database-generated), but
     * still appends {@code RETURNING id} so the generated value can be retrieved. Example output
     * for Dimension:
     *
     * <pre>
    ...
    private static SqlQueryWithFields getInsertSQL(Class<?> clazz) throws Exception {
        ...
        while (clazz != null) {
            ...
            for (Field field : fields) {
                logger.debug(
                        "Destructuring field: {} of entity class: {}",
                        field.getName(),
                        clazz.getName());
                boolean isId = field.isAnnotationPresent(Id.class);
                boolean isColumn = field.isAnnotationPresent(Column.class);
    ...
    /**
     * Generates a SQL DELETE statement for deleting an entity record by its ID. Inspects the entity
     * class to locate the @Id annotation and to find the table name from @Entity. The generated SQL
     * uses named parameters for the where clause.
     *
     * <p>Example output for Dimension: DELETE FROM dimensions WHERE id = :id;
     *
    ...
    /**
     * Generates a SQL SELECT statement for selecting an entity record by its ID. Inspects the
     * entity class to locate the @Id annotation and to find the table name from @Entity. The
     * generated SQL uses named parameters for the where clause.
     *
     * <p>Example output for Dimension: SELECT * FROM dimensions WHERE id = :id;
     *
     * @param clazz The entity class.
     * @return SqlQueryWithFields with list containing the ID field and the SQL string.
     * @throws Exception if @Entity/@Id annotation is missing or multiple @Id fields exist.
