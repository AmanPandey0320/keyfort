...
public class TableLoader implements DefaultLoader {
    ...
    private Mono<Credential> populateCredential(User u) {
        ...
        return this.credentialRepo
                .getAllActiveCredentialsByUser(u)
                .collectList()
                .flatMap(credentials -> {
                    for (Credential credential : credentials) {
                        if (credential.getIsActive()
                                && !credential.getIsDeleted()
                                && PasswordEncoderUtil.matches(
                                        this.password, credential.getHash())) {
    ...
    private Mono<User> populateUser(Client c) {
        ...
        return this.userRepo
                ...
                .flatMap(user -> {
                    if (user.isVerified()
                            && user.getClientId().equals(c.getId())
                            && !user.getIsDeleted()) {
                        logger.info(
                                "User with the user name exist, for the provided client, updating other info");
                ...
                .switchIfEmpty(Mono.defer(() -> {
                    ...
                    try {
                        logger.info(
                                "Saving new user with username: {}, as user don't exist",
                                this.userName);
                        return this.userRepo.save(u);
                    } catch (Exception e) {
                        throw new RuntimeException(e);
    ...
    private Mono<Client> populateClient(Dimension d) {
        ...
        return this.clientRepo.getClientsByDimension(d.getId()).collectList().flatMap(clients -> {
            if (clients.size() == 1) {
                logger.info(
                        "Single client present for the dimension id: {}",
                        d.getId().toString());
            ...
            logger.info(
                    "Multiple or no client found, creating fresh client, deleting old ones if any");
    ...
    private Mono<Dimension> populateDimension() {
        ...
        return this.dimensionRepo
                ...
                .doOnNext(d -> {
                    logger.info(
                            "Dimension present in database, creating one with name: {}, id is: {}",
                            this.dimensionName,
                            d.getId());
                })
                .switchIfEmpty(Mono.defer(() -> {
                    logger.debug(
                            "Dimension not present in database, creating one with name: {}",
                            this.dimensionName);
                    ...
                    try {
                        return this.dimensionRepo
                                .save(d)
                                .doOnNext(savedDimension -> logger.info(
                                        "Created dimension with id: {}", savedDimension.getId()));
                    } catch (Exception e) {
    ...
    public void loadData() {
        try {
            ...
            this.populateDimension()
                    ...
                    .subscribe(
                            d -> {
                                logger.info("Default values admin populated");
                            },
                            e -> {
