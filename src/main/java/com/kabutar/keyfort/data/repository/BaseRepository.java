...
public abstract class BaseRepository<T,C> {
    ...
    /**
     * function to get one from the db based on sql spec
     * and typecaste to Type C class entity object and return
     * a Mono
     * @param spec
     * @param clazz
     * @return
    ...
    protected Mono<C> getOne(DatabaseClient.GenericExecuteSpec spec, Class<C> clazz) {
        return spec.fetch().first().flatMap(row -> {
            try {
                ...
                throw new IllegalStateException("No constructor found in " + clazz.getName() 
                    + " that takes a " + row.getClass(), e);
            } catch (Exception e) {
